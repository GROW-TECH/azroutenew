...
export async function sendCoachApplicationNotification({ email, name, specialty, phone, location }) {
  const mailOptions = {
    from: process.env.EMAIL_FROM,
    to: email,
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <h2>Hello ${name},</h2>
        <p>Thank you for your interest in becoming a coach at Azroute Chess Institute!</p>
  ...
  try {
    ...
    console.error('Coach application email send error:', error);
    return { success: false, error };
  }
}
